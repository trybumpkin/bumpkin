...
from bumpkin.analysis.diffing import DEFAULT_IGNORES, DiffResult, build_diff, resolve_refs
...
from bumpkin.planner import PlannerDecision, plan_analysis_route
...
class PipelineContext:
    repo: str
    github_token: str
    event_context: orchestrator_scope.PREventContext
    notes: list[str]
...
def _resolve_scope_inputs(
    ...
    *,
    ...
    event_context: orchestrator_scope.PREventContext,
    notes: list[str],
) -> tuple[str, str, list[str] | None, str | None, bool]:
    from_ref_input, to_ref_input, scope_notes = orchestrator_scope.select_diff_scope(
        args.from_ref,
        args.to_ref,
        event_context,
    )
    ...
    if scope_guard_required and event_context.pr_number is not None:
        allowlist, fetch_error = orchestrator_scope.fetch_pr_changed_files(
            token=github_token,
            repo=repo,
        ...
        if allowlist is not None:
            notes.append(f"Scope guard loaded {len(allowlist)} PR file(s) from GitHub API.")
        elif fetch_error:
            notes.append(fetch_error)
    return from_ref, to_ref, allowlist, fetch_error, scope_guard_required


def _load_pipeline_config(
    args: Namespace, notes: list[str]
) -> tuple[BumpkinConfig, list[str], list[str], bool]:
    try:
    ...
    return bumpkin_config, ignore_patterns, public_api_hints, use_difftastic
...
def _build_scope_guard(
    *,
    ...
    mismatch_detected, mismatch_reason = orchestrator_scope.evaluate_scope_mismatch(
        required=required,
    ...
    scope_guard: dict[str, object] = {
        "required": required,
        "source": "github-pr-files" if allowlist is not None else "unavailable",
        "fetch_error": fetch_error,
        "pr_files_count": len(allowlist or []),
        "git_files_count": diff_result.changed_files_total,
        "overlap_count": diff_result.scope_overlap_files,
        "unexpected_count": diff_result.scope_unexpected_files,
        "missing_count": diff_result.scope_missing_files,
    ...
    if mismatch_detected and mismatch_reason:
        notes.append(f"Scope mismatch guard triggered: {mismatch_reason}.")
    return scope_guard, mismatch_detected, mismatch_reason


def _build_prompt_and_plan(
    args: Namespace,
    *,
    ...
    models_token: str,
    notes: list[str],
) -> tuple[PromptPackMetadata, list[str], PlannerDecision]:
    detected_language_groups = detect_language_groups(diff_result.analyzed_files)
    prompt_language_group = (
        detected_language_groups[0] if len(detected_language_groups) == 1 else "generic"
    )
    ...
    return prompt_metadata, language_hints, planner_decision
...
def _prepare_pipeline_context(
    ...
    *,
    ...
    notes: list[str] = []
    from_ref, to_ref, allowlist, fetch_error, scope_guard_required = _resolve_scope_inputs(
        args,
        repo=repo,
    ...
    config, ignore_patterns, public_api_hints, use_difftastic = _load_pipeline_config(args, notes)
    ...
    scope_guard, mismatch_detected, mismatch_reason = _build_scope_guard(
    ...
    prompt_metadata, language_hints, planner_decision = _build_prompt_and_plan(
        args,
        config=config,
        diff_result=diff_result,
    ...
    return PipelineContext(
        ...
        event_context=event_context,
        notes=notes,
        bumpkin_config=config,
        diff_result=diff_result,
        scope_guard=scope_guard,
        scope_mismatch_detected=mismatch_detected,
        scope_mismatch_reason=mismatch_reason,
...
def run(args: Namespace, *, comment_poster: CommentPoster | None = None) -> int:
    ...
    context = _prepare_pipeline_context(
        args,
        repo=repo,
        github_token=github_token,
        event_context=event_context,
    ...
    core_result = orchestrator_core.analyze_diff_core(
        ...
        endpoint=args.models_endpoint,
        token=context.models_token,
        max_retries=args.max_retries,
        request_timeout=getattr(args, "request_timeout", 45),
        prompt_metadata=context.prompt_metadata,
        bumpkin_config=context.bumpkin_config,
        planner_decision=context.planner_decision,
        notes=context.notes,
        event_labels=event_context.labels,
        scope_mismatch_detected=context.scope_mismatch_detected,
        scope_mismatch_reason=context.scope_mismatch_reason,
        scope_guard=context.scope_guard,
        public_api_hints=context.public_api_hints,
        language_hints=context.language_hints,
    )
