...
def test_pipeline_routes_multilanguage_diffs_to_generic_pack(monkeypatch) -> None:
    monkeypatch.setenv("GITHUB_REPOSITORY", "acme/repo")
    monkeypatch.setenv("GITHUB_TOKEN", "token-123")
    monkeypatch.setenv("BUMPKIN_CAPTURE_PR_COMMENT_ONLY", "1")

    requested_language_groups: list[str | None] = []
    ...
    monkeypatch.setattr(
        "bumpkin.orchestrator.pipeline.orchestrator_scope.fetch_pr_changed_files",
        lambda **_kwargs: (["src/api.ts", "pkg/api.py"], None),
    )
    monkeypatch.setattr(
        "bumpkin.orchestrator.pipeline.load_bumpkin_config",
        orchestrator_pipeline._fallback_config,
    ...
    monkeypatch.setattr(
        ...
        lambda **_kwargs: SimpleNamespace(
            notes=[],
            analyzed_files=["src/api.ts", "pkg/api.py"],
            approx_prompt_tokens=128,
            changed_files_total=2,
            scope_overlap_files=2,
            scope_unexpected_files=0,
            scope_missing_files=0,
        ),
    )
    ...
    monkeypatch.setattr(
        "bumpkin.orchestrator.pipeline.detect_language_groups",
        lambda _files: ["javascript-typescript", "python"],
    )
    monkeypatch.setattr("bumpkin.orchestrator.pipeline.detect_language_hints", lambda _files: [])

    def _get_prompt_metadata(**kwargs: Any) -> SimpleNamespace:
        requested_language_groups.append(kwargs.get("language_group"))
        return SimpleNamespace(language_group="generic", promotion_status="experimental")
    ...
    assert exit_code == 0
    assert requested_language_groups == ["generic"]


def test_pipeline_threads_language_hints_into_core(monkeypatch) -> None:
    monkeypatch.setenv("GITHUB_REPOSITORY", "acme/repo")
    monkeypatch.setenv("GITHUB_TOKEN", "token-123")
    monkeypatch.setenv("BUMPKIN_CAPTURE_PR_COMMENT_ONLY", "1")

    captured_language_hints: list[str] | None = None
    ...
    monkeypatch.setattr(
        "bumpkin.orchestrator.pipeline.detect_language_hints",
        lambda _files: ["For Python, treat __all__ exports as public API candidates."],
    )
    monkeypatch.setattr(
        "bumpkin.orchestrator.pipeline.get_prompt_metadata",
        lambda **_kwargs: SimpleNamespace(language_group="python", promotion_status="experimental"),
    ...
    def _analyze_diff_core(**kwargs: Any) -> SimpleNamespace:
        nonlocal captured_language_hints
        captured_language_hints = kwargs.get("language_hints")
        return SimpleNamespace(
            output={"status": "classified", "label": "PATCH", "confidence": "high"},
            result={"status": "classified", "label": "PATCH", "confidence": "high"},
            notes=kwargs["notes"],
    ...
    assert exit_code == 0
    assert captured_language_hints == [
        "For Python, treat __all__ exports as public API candidates."
    ]
